...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-600 mb-4">Get started with your personal details.</p>
      ...
      <div className="flex items-center gap-3">
        <label className="cursor-pointer">
            ...
            <img
              src={
                typeof data.image === "string"
                  ? data.image
                  : URL.createObjectURL(data.image) // ✅ fixed
              }
              alt="user-profile"
              className="w-16 h-16 rounded-full object-cover mt-3 ring ring-slate-300 hover:opacity-80"
            />
          ) : (
            <div className="inline-flex items-center gap-2 mt-3 text-slate-600 hover:text-slate-700 cursor-pointer">
          ...
          <input
            type="file"
            accept="image/jpeg, image/png" // fixed typo
            className="hidden"
            onChange={(e) => handleChange("image", e.target.files[0])}
          />
        </label>

        {/* ✅ Remove background toggle (only shown after upload) */}
      ...
      {fields.map((field)=>{
        ...
        return(
            <div key={field.key} className="sapce-y-1 mt-5">
